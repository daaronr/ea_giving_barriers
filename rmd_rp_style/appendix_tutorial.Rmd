...
['hypothes.is'](https://web.hypothes.is/) is an easy way to do so (please write directly in the html and contact me at daaronr at gmail dot com to let me know you've done so).
...
-   **R** is a free programming language which is mainly used for data
    ...
    command.^[If you are familiar with Python, these R packages are roughly
...
-   The **Bookdown** package was built on the Rmarkdown package, but it
    ...
    becoming its own HTML page.^[Look at the list of headings on the left of this page: each second-level
...
`readme.md` files.^[Actually there are several varieties of markdown, but they mainly share
key elements.
]
...
users to easily display objects such as plots or tables of data.^[Text can be made *italic* using single asterisks `*italic*` and **bold**
...
-   **Margin notes** Margin notes are used throughout these projects as
    ...
    following HTML wrapper:^[The margin notes used in this project are inspired by the Tufte handout
...
describing the latest comments.^[Once you've installed Git you will want to 'clone this repo' to have it
...
-   in the file 'sections/inertia.rmd' a space has been added after
    'crowding out?'^[This is but one way to view and consider changes. Various text
...
How to think about Airtable, learning it without getting overwhelmed...
::: {.foldable} 
...
![barriers to EAG](picsfigs/barriers_to_EAG.png)^[Again, this is older work, maybe already incorporated into the Bookdown?
]
