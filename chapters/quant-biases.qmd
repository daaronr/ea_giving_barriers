# Quantitative biases (the 'distorted altruist') {#quant-biases}
...
@caviola2020many (literature review) considers ["Epistemic Obstacles"](https://www.cell.com/trends/cognitive-sciences/fulltext/S1364-6613(21)00090-5#secst0060), including 'Innumeracy'. They give a few examples here, including 'psuedoinefficacy' ('drop in the bucket', ['proportional dominance'](#prop-dom)) here. (However, they consider 'scope neglect' a 'motivational obstacle'.)
...
## Proportional dominance effect {#prop-dom}^[AKA 'drop in the bucket', 'psychosocial numbing', 'psychophysical numbing'; relates to['Pseudoinefficacy'](https://www.arithmeticofcompassion.org/pseudoinefficacy)
]
...
### Conceptual Discussion^[Overview of findings from papers, caveats, how the concept works, etc. Provides context for evidence section; Discussion of the relevant mechanisms at play; Discussion of the relevant established theories.
]
...
### EG Relevance^[How this particular barrier proves problematic for effective giving.
]
...
### State of Evidence^[Key papers: Summarize findings and key takeaways, Short description of methods for relevant studies, Make sure to include both description of evidence and evaluation of evidence
]

#### @fetherstonhaughInsensitivityValueHuman1997 {-}
...
**Replication evidence**: Mixed, not strongly supported; see @zianoNumbingSensitizationReplications2021


\


#### @erlandssonEmotionalReactionsPerceived2015  (Study 4) {-}
...
Hypothetical willingness to pay (wtp) questions. Within-subject manipulations only; standard student subject pools, small samples.^[They do reverse order of presentations for half the participants. They report a lack of significant order effects, but fail to discuss the power of such tests or examine first-presented choices in isolation.
]
...
## Statistical/identifiable victim effect {#statvictim}^[Cite: Hsee13, small2003helping, kogut2011identifiable, @Small2007 (studies 3-4)
]

### Definition

The identifiable victim effect (IVE) is credited as being first developed by @schelling1968life, who argue "there is a distinction between an individual and a statistical life". People may react differently towards victims when these victims are *identified* rather than anonymous [@smallHelpingVictimHelping2003]. As a result, resources may be more likely to be devoted to causes where individual victims are more prominent and identifiable (rather than cases where we only see large, unidentified groups) [@small2010reference]. @slovicPsychicNumbingGenocide2007 also discussed this effect; this paper derives its title from a quote attributed to Mother Teresa:

> If I look at the mass I will never act. If I look at the *one* I will.

^[Jason Schukraft notes:
> In contrast, Bertrand Russell is alleged to have said "The mark of a civilized human is the ability to look at a column of numbers, and weep."

(Aside: it's probably a misattribution, though it's often repeated. At the very least Russell endorsed the claim.)
]


### Relevance to effective giving

@baron2011heuristics discuss how the IVE could pose a problem for effective altruism, suggesting two reasons that this could be considered a bias.^[ At least narrowly speaking, this could be seen as a 'bias', not only from the utilitarian perspective, but more generally. Presumably, no moral theory would hold that the moral status of an individual, or one's obligation to them should depend on how that individual is *described*.  (Thanks to Jason Schukraft for this point).
]

Firstly, they suggest that when altruism is increased for identified victims, this could result in less altruism for those who are not identified,$^{(?)}$^[(?) Why? Through which channel.
]
...
### Evidence: Meta-analysis and reviews/surveys

@leeIdentifiableVictimEffect2016: Meta-analysis
...
See also, the discussion in @kogut2011identifiable
...
In one of their studies, they used a version of the "dictator game". In dictator games, the first player, known as the 'allocator' is given an endowment. They then make a unilateral decision on how they should split this endowment. The second player, known as the 'recipient', has to accept this split.^[For more on dictator games see @kahneman1986fairness and @camerer1995anomalies.
]
...
Their first finding was that "the effect of identification may be largely restricted to single victims", as the impact of being identified was significantly less for the group of eight children than for the single child. Secondly, they found that "the identification of the single victim is more effective the more vivid the representation", with willingness to donate at its highest when the victims were identified by age, name, and picture.^[I'd like to check the robustness of this 'interaction' effect; such effects may be confounded with non-linearity. See discussion [here, section 1.3.7](https://daaronr.github.io/metrics_discussion/introduction.html#basic-regression-and-statistical-inference-common-mistakes-and-issues)
]
...
Most people in developed countries like the US and the UK will have an experience with cancer over their lifetime, either personally or through a loved one.^[It is estimated that roughly [38% of men and women](https://www.cancer.gov/about-cancer/understanding/statistics#:~:text=Approximately%2038.4%25%20of%20men%20and,1%2C790%20died%20of%20the%20disease.) will be diagnosed with cancer at some point over their lifetime.
]
...
If cancer is more *visible*, it could be more *salient*,leading people to overestimate its importance through their use of the availability heuristic.^[Note that none of the 'barriers' we mention are mutually exclusive. There may be many other factors driving the popularity of cancer research charities.
]
...
<!-- JS:  But what if you are uncertain which charity this is? Personally, I donate to multiple cause areas because I'm uncertain which cause area is best. Is that form of diversification irrational? -->
...
Schukraft: Is there any evidence about how people actually partition charities? It seems like for partition dependence to be a barrier to effective giving, we have to assume that people are going to more finely individuate non-effective charities than effective charities. (That's plausible, but I'd like to see some evidence to support it.) If the opposite is true, then partition dependence would actually lead to more effective giving.

DR: thinking about it. The case could be:

To optimize, I should choose charities without any regard to these irrelevant concerns. Thus any 'noise' that enters in will take me away from efficiency.

If people who would otherwise choose efficiently are diversifying to mix in more of what the majority do, this will tend to make them less efficient.

But I see your point, and 'adding noise' to people who are already suboptimal could make them more optimal
...
## Overhead aversion {#overhead-aversion}
...
Studies finds 'no correlation' between overhead and effectiveness-- are these convincing?^[Check claims made in "BBB Wise Giving Alliance"; Caviola et al., 2014; Gregory & Howard, 2009; Karlan, 2014; Pallotta, 2008; Sellers, 2018
]
...
<!-- JS: I grant this theoretical argument, but as a matter of empirical fact, do effective charities have abnormally high overhead? Seems like something that could be reasonably checked via Charity Navigator.
-->
...
<!-- JS: Anecdotally, my impression is that this already happens for most major charities, both effective and ineffective. (Obviously would be good to have actual empirical evidence to support this impression, though.) --> 
...
<!-- JS:  Agreed that this is technically a sunk cost, but number of deaths might be a decent proxy for the magnitude of the disaster and the magnitude of the disaster might correlate pretty well with the needs of the survivors. -->
...
## Scope insensitivity/embedding effect/part-whole effect {#scope-insens}


<!-- JS: Surprised this section is so short! My naïve impression is that this is one of the biggest (maybe the biggest) barrier to effective giving!
--> 
::: {.alert .alert-secondary}
...
Incorporate notes from [this comment](https://forum.effectivealtruism.org/posts/oYvmPnc72npdtFuPg/effects-of-being-able-to-see-effects-of-donation-first-hand?commentId=CyeKP5TsdeNhLCFd2).

Reprinted in fold.

::: {.callout-note collapse='true'}
 to make donations sustainable in the large community one might need to think about *both*
...
*Intervention A (e.g., bednets in country A)*: Saves one life, on average, per 5000 USD donated and used. However, for every 1 USD donated, 5 cents must be paid in bribes to enable the bednets to be distributed. Suppose--just for this example--that the paying these bribes has no other negative consequences in the short or long term.^[This may be hard to swallow. We could also consider that the negative consequences of these bribes has already been taken into account and considering the effectiveness.
]
