...
Text can be made *italic* using single asterisks `*italic` **bold** by using asterisks `**bold**`.
...
### Inline code
Inline code is a quick and easy way to put snippets of R code. As an alternative to using code chunks, R code can simply be placed inside of ``` `r ` ```. For example, this can be an easy way to insert the value of a variable into a paragraph without inserting a chunk.

### Latex/maths
R Markdown makes use of the LaTeX document preparation system, which is popular for writing technical documents with mathematical content. This allows us to publish documents which include equations such as:
...
### Margin notes
Margin notes are used throughout projects as a way of displaying information in an organised and aesthetically pleasing way. The formatting for margin notes as well as other visual features are outlined in [tufte_plus.css](support/tufte_plus.css).
To add a margin note, text is placed inside the following HTML wrapper.
...
Pandoc provides a way to generate formatted references as well as a bibliography in Markdown. The bibliography file is specified in the associated YAML metadata file, which in this case is [bookdown.yml](writing_econ_book/bookdown.yml).

- **BibTeX**

The BibTeX format refers to a stylized file format which is used predomoninantly for lists of references. BibTeX bibliographies use the .bib extension. For example the bibliography for this project is [giving_keywords.bib](support/giving_keywords.bib). For more information on BibTeX see [here](http://www.bibtex.org/)
