...
bibliography: ["support/reinstein_bibtex.bib", "support/reinstein_references.bib"] #this first one should be included in the citr settings; not sure what the second one is
...
- Baron and Szymanowski 2011 - Heuristics and Biases in Charity: Largely conceptual, minimal survey of specific empirical/experimental papers
...
If we *could* shift the 'meaningful giving' \(\rightarrow\) we could use theory of Conditional Demand (Pollak) \(\rightarrow\) "expenditure substitution" [@reinsteinDoesOneCharitable]. Still, the sign of the conditional demand effects are straightforward only under very restrictive conditions. 
...
- But can we rule out *direct* effects of the first promotion? (see Heckman/Pinto on "mediators")
...
With quadratic utility, the partial derivatives will be constants,
...
Social signaling is seen to be a major driver of human behavior, particularly in the charitable domain.^[REF]^ Essentially, we may make choices that we would not have otherwise made in order to boost our reputation among our peers, colleagues, etc.. Reputation may be valued for its own sake or instrumentally, as a means  to inducing others to act more favorably to us. Game theory (see esp. [@spenceJobMarketSignaling1973]) offers a precise conception of this signaling as a costly way to demonstrate one's positive "type"  in a context with asndymmetric information.
...
Background mechanism:  donors’ failure to prioritize cost-effectiveness can be explained by signaling concerns, since people who favor deontological over consequentialist decisions are preferred as social partners (Everett et al., 2016),
...
Thus, there may be reputational benefits associated with deliberating. However, since deliberators are less preferred as social partners the authors believe the overall effect one reputation is negative. They do not find any differences in trustworthiness. Exhibiting empathy before deliberating reduced most negative reputational effects. People how do not give at all are seen as the worst. Surprisingly, participants judge the empathy donors as more wastful.
...
I am testing how to add some regular text here, OK, maybe I will put something in *italics*.
...
<div class="marginnote">
This should probably be moved to the nextg section
</div>
...
[@Kogut2011; @Loewenstein2007; @Small2003:1; @Small2007; @Small2007a]
 (Drouvelis & G, ’16), (Caviola ea, ’14)
...
Misc -- adding a bunch of citations; please ignore:
...
Pandoc provides a way to generate formatted references as well as a bibliography in [R-Markdown](https://rmarkdown.rstudio.com/authoring_bibliographies_and_citations.html).
...
When using the Citr package to add citations to projects such as this one, make sure to have Citr update the correct Bibtex file. 
...
```{r, out.width='85%', fig.align='center', fig.cap='EA barriers repo github starting page'}
...
![Commenting on a part of a commit, notifying a collaborator](picsfigs/commenting_a_commit.png)
...
This project is closely connected to [innovationsinfundraising.org](innovationsinfundraising.org). Much of these projects overlap, and there is a shared 'database' stored as an airtable [Giving researchers shared](https://airtable.com/invite/l?inviteId=invjWg63fpQAOBcch&inviteToken=0b154f62460aba428e0bf8c082ae3947e2e9b62b1045994a767eee551bb92833)


We had an earlier ... [tutorial on using the Airtable and Innovationsinfundraising.org here](https://github.com/daaronr/ea_giving_barriers/blob/master/documentatiion_and_tutorials/project_airtableexplained/airtable_database_overview.pdf)
...
![barriers to EAG, why relevant](picsfigs/barriers_to_EAG_whyrelevant.png)
